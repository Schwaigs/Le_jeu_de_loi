...
class Heritage {
    ...
    public function chercherHeritier(){
        ...
        $resultPerso = MyPDO::pdo()->prepare("SELECT id,parent,religion,sexe FROM personnage WHERE classe not in ('mort','roi') and age>10");
        $resultPerso->execute();
        ...
        //si aucun des personnages de la base ne correspond aux lois on a pas d'heritiers le joueur à perdu
    ...
    public function meilleurSante(array $heritiers) : int {
        ...
        $in_heritiers = implode(',',$heritiers);
        $resultSante = MyPDO::pdo()->prepare("SELECT id,etatSante from personnage WHERE id in (".$in_heritiers.")");
        $resultSante->execute();
